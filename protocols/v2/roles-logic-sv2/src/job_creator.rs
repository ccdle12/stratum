...
fn new_extended_job(
    ...
    println!("CCDLE12 DEBUG: @ received script_prefix: {:?}", script_prefix);
    ...
    // TODO TMP: My theory is if I use the first byte to read it, it should
    // cover all cases of different block heights.
    println!("SCRIPT PREFIX: {:?}", &script_prefix);
    let bip34_len = script_prefix[1] as usize;
    println!("SCRIPT PREFIX: @ bip34_len {:?}", &bip34_len);
    let bip34_bytes = script_prefix[1..2 + bip34_len].to_vec();
    println!("SCRIPT PREFIX: @ bip34_bytes{:?}", &bip34_bytes);
...
fn coinbase_tx_prefix(
    ...
    let r = encoded
        ...
        // TODO: Technically this needs to be:
        // 0...
        // 4 byte version
        // 2 byte optional segwit flag
        // 1 byte number of inputs
        // 3 bytes of something??
        // 32 bytes of txid
        // 4 bytes of ouputs
        // 1 byte script sig size
        // 1 byte block height bytes
        // 3 bytes (variable according to above height) of block height
    println!("CCDLE12 DEBUG: @ sliced prefix: {:?}", &r);
...
pub fn on_new_template(
    ...
    println!("CCDLE12 DEBUG: template.coinbase_tx_value_remaining: {:?} and self.block_reward_staoshi: {:?}", &template.coinbase_tx_value_remaining, &self.block_reward_staoshi);

    // TODO: Why is this a thing??? - Shouldn't it assigning this everytime?, lets just make it
    // 0 block reward for now
    ...
    if template.coinbase_tx_outputs_count > 0 {
        ...
        println!("CCLDE12 DEBUG: WitnessCommitment: {:?}", &witness_commitment);
        println!("CCLDE12 DEBUG: self.coinbase_outputs: {:?}", &self.coinbase_outputs);
        self.coinbase_outputs.push(witness_commitment);
    }

    // TODO: This going to lead to the large amount of outputs
    // TODO: Add out own coinbase tx output but then also add the witness committment, so I
    // need to be able to convert the B064K into a TxOut?.
    ...
    println!("CCLDE12 DEBUG: self.coinbase_tx_outputs size: {:?}", &self.coinbase_outputs.len());
